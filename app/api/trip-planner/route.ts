export async function POST(request: Request) {
  try {
    ...
    if (!destination || !startDate || !endDate) {
      return Response.json({ error: "Destination, start date, and end date are required" }, { status: 400 })
    }
      ...
      tripPlan = {
        ...
        dailyItinerary: Array.from({ length: duration }, (_, i) => ({
          ...
          accommodation: {
            ...
            location: `${destination} city center`,
            checkIn: "3:00 PM",
            cost: budget === "Budget" ? "$50/night" : budget === "Luxury" ? "$200/night" : "$100/night",
          },
        })),
        accommodationDetails: [
          {
            ...
            type: "Hotel",
            location: `${destination} city center`,
            pricePerNight: budget === "Budget" ? "$50" : budget === "Luxury" ? "$200" : "$100",
        ...
        budgetBreakdown: {
          ...
          transportation: "$800",
          food: `$${duration * 60}`,
          activities: `$${duration * 80}`,
          shopping: `$${duration * 30}`,
          miscellaneous: `$${duration * 20}`,
          total:
            budget === "Budget"
              ? `$${duration * 100}`
              : budget === "Luxury"
                ? `$${duration * 500}`
                : `$${duration * 250}`,
          dailyAverage: budget === "Budget" ? "$100" : budget === "Luxury" ? "$500" : "$250",
        ...
        emergencyInfo: {
          localEmergency: "Local emergency services: 911",
